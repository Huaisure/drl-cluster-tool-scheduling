...
def _build_robot_contexts(trucks, state_trucks):
    ...
    for truck in trucks:
        ...
        start_time = state_info.get("bound_time")
        contexts[truck_id] = {
            "start_time": int(start_time) if start_time is not None else None,
            "start_location": state_info.get("location"),
            "end_time": None,
        }
    return contexts
...
def close_down(
    ...
    close_down_time = model.NewIntVar(0, upper_bound, "close_down_time")
    ...
    action_times = _create_action_time_vars(model, nodes, upper_bound, action_active, close_down_time)
    _enforce_goods_priority_for_actions(
        model,
        nodes,
        action_active,
    ...
    # 创建额外区间：末态 PM unload 区间（StartTime<0，负时间直接使用）
    ...
    for ei in target_state.get("process_unload_intervals") or []:
        ...
        iv = model.NewFixedSizeIntervalVar(orig_start, duration, f"extra_iv_{len(extra_ivs)}")
        extra_ivs.append(iv)
    ...
    model.Minimize(close_down_time)
    ...
    result = build_solution_result(
        solver=solver,
        status=status,
        objective_name="close_down_time",
        objective_value=solver.Value(close_down_time),
        cpu_time=elapsed,
        nodes=nodes,
...
def _build_close_down_save_path(save_dir: str, recipe_path: str) -> str:
    """根据 recipe 名称自动构建 close_down 保存路径"""
    recipe_name = Path(recipe_path).stem
    return str(Path(save_dir) / recipe_name / "close_down.json")


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--recipe-path", default=r"recipes\1.json")
    parser.add_argument("--state-path", default=r"results\1\end_state.json")
    ...
    if args.save_path:
        ...
        save_path = _build_close_down_save_path(args.save_dir, recipe_path)
    ...
    result = _unscale_schedule(result, time_scale, "close_down_time")
