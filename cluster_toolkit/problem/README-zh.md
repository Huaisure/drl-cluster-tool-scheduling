...
问题JSON直接保存每片Wafer在加载阶段已经计算好的`priority`，不保存策略名称，也不在
Engine中执行策略脚本。`priority`是必填的非负整数，数值越小，首次投片优先级越高。
优先级如何由slot、任务、Recipe或业务规则计算，属于Problem加载前的数据准备流程。

领域生成器会把已经计算完成的逐片priority写入JSON，并覆盖“全部相同”“按Recipe分组”
和“Recipe内分批”三类结果，但不保存策略名称。Engine只在虚拟IO的首次Pick中强制全局
最小priority；进入设备后的Pick/Place不使用该字段。

## Schema版本与工艺能力

未写`schema_version`的旧JSON按version 1解析，继续以RouteVisit中的
`module_id/module_ids`为权威候选集合。新领域生成器输出`schema_version: 2`：

- Module类型增加`AL`和`BUFFER`；
- PM通过`process_ids`声明已配置工艺；
- PM RouteVisit通过`process_id`声明工艺需求；
- 该步骤的`module_ids`必须恰好等于所有支持该工艺的PM；
- AL、LL和BUFFER RouteVisit不设置`process_id`。

因此旧数据无需迁移；只有version 2才启用工艺能力一致性校验。
...
- `priority`是必填非负整数，数值越小，首次投片越优先；
- `step_index=0` 表示虚拟IO，`1..N`表示Route中的步骤，`N+1`表示最终返回IO；
- `process_end_time` 表示时间轴从0开始时，当前加工预计结束的时刻；`null`或`0`表示当前没有未完成的加工。
- 新RL问题必须只有一个虚拟IO，IO不能出现在Route中。所有Wafer隐式返回该IO，因此不写
  `return_lp_id`。旧Validator输入仍兼容真实LP以及`return_lp_id`，但同一Problem不能混用
  虚拟IO和LP。
...
- Module容量默认由类型派生：legacy LP为25，其余Module为1；虚拟IO应显式设置为
  Wafer总数。问题JSON显式提供
