...
class ModuleType(str, Enum):
    ...
    BUFFER = "BUFFER"
    IO = "IO"
    # Legacy validator inputs may still contain physical LP modules. New RL
    # problems use one virtual IO and may not mix IO with LP.
    LP = "LP"
...
class Module(_StrictModel):
    ...
    @classmethod
    def _validate_process_ids(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        for process_id in value:
            _require_non_empty(process_id, "Module.process_ids item")
        if len(set(value)) != len(value):
            raise ValueError("Module.process_ids must not contain duplicates")
        return value
...
class RouteVisit(_StrictModel):
    ...
    @classmethod
    def _validate_process_id(cls, value: str | None) -> str | None:
        if value is None:
            return None
        return _require_non_empty(value, "RouteVisit.process_id")
...
class ClusterProblem(_StrictModel):
    ...
    def _validate_references(self) -> "ClusterProblem":
        ...
        for route_id, route in self.routes.items():
            for visit in route.visits:
                candidate_types: set[ModuleType] = set()
                for module_id in visit.module_ids:
                    if module_id not in self.Modules:
                        raise ValueError(f"Route {route_id} references unknown Module: {module_id}")
                    candidate_types.add(self.Modules[module_id].type)
                    if self.Modules[module_id].type is ModuleType.IO:
                        raise ValueError(
                            f"Route {route_id} must not contain virtual IO Module {module_id}"
                        )
                if self.schema_version >= 2:
                    if len(candidate_types) != 1:
                        raise ValueError(
                            f"Route {route_id} visit candidates must share one Module type"
                        )
                    candidate_type = next(iter(candidate_types))
                    if candidate_type is ModuleType.PM:
                        if visit.process_id is None:
                            raise ValueError(
                                f"Route {route_id} PM visit must define process_id in schema version 2"
                            )
                        configured = {
                            module_id
                        ...
                        if set(visit.module_ids) != configured:
                            raise ValueError(
                                f"Route {route_id} process {visit.process_id} candidates "
                                "must equal all configured PM Modules"
                            )
                    elif visit.process_id is not None:
                        raise ValueError(
                            f"Route {route_id} non-PM visit must not define process_id"
                        )

        io_ids = self.io_module_ids
        if len(io_ids) > 1:
            raise ValueError("a problem may contain at most one virtual IO Module")
        if io_ids and any(
            module.type is ModuleType.LP for module in self.Modules.values()
        ):
            raise ValueError("virtual IO and legacy LP Modules must not be mixed")

        for module_id, module in self.Modules.items():
            if self.schema_version >= 2:
                if module.type is ModuleType.PM and not module.process_ids:
                    raise ValueError(
                        f"PM Module {module_id} must define process_ids in schema version 2"
                    )
                if module.type is not ModuleType.PM and module.process_ids:
                    raise ValueError(
                        f"non-PM Module {module_id} must not define process_ids"
                    )
    ...
    def _validate_initial_state(self) -> None:
        ...
        for wafer in snapshot.wafers_by_key.values():
            ...
            if isinstance(wafer.location, ModuleLocation):
                ...
                if wafer.return_lp_id is not None or (
                    lp_count > 1 and module.type is not ModuleType.LP
                ):
                    self.return_module_id(wafer)
                continue
            ...
            if isinstance(wafer.location, RobotLocation):
                ...
                if wafer.return_lp_id is not None or lp_count > 1:
                    self.return_module_id(wafer)
    ...
    def return_module_id(self, wafer: WaferInitialState) -> str:
        """Resolve the prescribed final IO or legacy LP for one wafer."""

        if self.io_module_ids:
            if wafer.return_lp_id is not None:
                raise ValueError(
                    f"initial_state wafer {wafer.wafer_key!r} must not define "
                    "return_lp_id in a virtual IO problem"
                )
            return self.io_module_ids[0]

        if wafer.return_lp_id is not None:
            module = self.Modules.get(wafer.return_lp_id)
            if module is None:
                raise ValueError(
                    f"initial_state wafer {wafer.wafer_key!r} return_lp_id "
                    f"references unknown Module: {wafer.return_lp_id}"
                )
            if module.type is not ModuleType.LP:
                raise ValueError(
                    f"initial_state wafer {wafer.wafer_key!r} return_lp_id "
                    f"must reference an LP: {wafer.return_lp_id}"
                )
            return wafer.return_lp_id

        if isinstance(wafer.location, ModuleLocation):
            module = self.Modules[wafer.location.module_id]
            if module.type is ModuleType.LP:
                return wafer.location.module_id
        ...
        if len(lp_ids) == 1:
            return lp_ids[0]
        raise ValueError(
            f"initial_state wafer {wafer.wafer_key!r} must define "
            "return_lp_id when it does not start in an LP and the "
            "problem has multiple LP modules"
        )

    @property
    def io_module_ids(self) -> tuple[str, ...]:
        return tuple(
            module_id
            for module_id, module in self.Modules.items()
            if module.type is ModuleType.IO
        )
    ...
    def _validate_initial_module_step(
        ...
        if module_type in {ModuleType.IO, ModuleType.LP}:
            valid_terminal_steps = {0, len(route.visits) + 1}
            if wafer.step_index not in valid_terminal_steps:
                raise ValueError(
                    f"initial_state wafer {wafer.wafer_key!r} in terminal Module "
                    f"{module_id} has invalid step_index: "
                    f"{wafer.step_index}"
                )
            return
